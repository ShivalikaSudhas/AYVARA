...
export default function DispatcherConsole() {
  ...
  const handleConfirmCitizenReport = (reportId: string, desc: string) => {
    setCondition(desc);
    setCitizenReports((prev) => prev.filter((r) => r.id !== reportId));
    alert(`Report ${reportId} confirmed! Converted to official Emergency Request for dispatch.`);
  };
  ...
  return (
    <div className="min-h-screen bg-[#F6F8F6]">
      ...
      <main className="ml-64 p-8">
                ...
                >
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-slate-400">· {report.phone}</span>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => handleConfirmCitizenReport(report.id, report.description)}
        ...
        {/* Emergency Dispatcher Triage Form */}
        <div className="mt-8 rounded-2xl border border-[#DDE5DF] bg-white p-6 shadow-sm">
          ...
          <form onSubmit={handleExecuteDispatchMatch} className="mt-4 space-y-4">
            ...
            >
              <Sparkles size={16} />
              {loading ? "Calculating..." : "Run P3 Smart Match & Dispatch"}
            </button>
          </form>
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      ICU Beds: {m.available_icu_beds} Free | Blood Stock: {m.has_blood_stock ? "Available" : "Depleted"}
                    </p>
                  </div>

                  <button
                    onClick={() => alert(`Dispatched Ambulance Unit A-09 to ${m.hospital_name}!`)}
                    className="flex items-center gap-1 rounded-xl bg-green-700 px-4 py-2 text-xs font-bold text-white hover:bg-green-800"
                  >
                    <Plus size={14} />
