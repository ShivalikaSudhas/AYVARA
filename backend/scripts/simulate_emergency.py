...
def run_simulation():
    ...
    try:
        ...
        if login_res.status_code == 200:
            ...
            print(f"[SUCCESS] Authenticated as DISPATCHER! JWT Token: {disp_token[:30]}...")
        else:
            print("[NOTICE] API Offline. Simulating local token generation for rehearsal.")
            disp_token = "mock_dispatcher_jwt_token"
    except Exception as e:
        print(f"[NOTICE] API Connection Note: {e}. Running rehearsal with local mock fallback.")
        disp_token = "mock_dispatcher_jwt_token"
    ...
    top_match = matches[0]
    print(f"[RESULT] P3 MATCH ENGINE SCORING:")
    ...
    print(f"  * ICU Beds Available: {top_match['available_icu_beds']}")
    ...
    print(f"[SUCCESS] RESERVATION CONFIRMED!")
    ...
    time.sleep(1)
    print(f"[SUCCESS] DISPATCH EN ROUTE!")
    print(f"  - Ambulance Unit: Unit A-09 (ALS Cardiac Prepared)")
    print(f"  - Status: EN_ROUTE -> ARRIVED AT SCENE -> PATIENT LOADED")
    ...
    time.sleep(1)
    print(f"[SUCCESS] ARRIVAL CONFIRMED!")
    print(f"  - Target Bed (ICU-Bed-04) status converted: RESERVED -> OCCUPIED")
    print(f"  - Final Transfer Lifecycle Status: COMPLETED")

    print("\n======================================================================")
    print("[COMPLETED] CRITICAL EMERGENCY SCENARIO SIMULATION REHEARSAL")
    print("======================================================================\n")
